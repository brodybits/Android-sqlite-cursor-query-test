...
import com.test.sqlc.SQLiteDebug.DbStats;
...
// XXX TODO MOVE:
    if (!isLibLoaded) {
      ...
      isLibLoaded = true;
        ...
        Log.i(TAG, "Opening database at path: " + mConfiguration.path);

        //mConnectionPtr = nativeOpen(mConfiguration.path, mConfiguration.openFlags,
        //        mConfiguration.label,
        //        SQLiteDebug.DEBUG_SQL_STATEMENTS, SQLiteDebug.DEBUG_SQL_TIME);

        // XXX TODO honor the openFlags!!
        long mydbc = SQLiteNative.sqlc_db_open(mConfiguration.path, SQLiteNative.SQLC_OPEN_READWRITE | SQLiteNative.SQLC_OPEN_CREATE);
        if (mydbc < 0) throw new RuntimeException("SQLite open error code: " + (-mydbc));
        mConnectionPtr = mydbc;
    ...
    private void dispose(boolean finalized) {
        ...
        if (mConnectionPtr != 0) {
            ...
            try {
                ...
                int err = SQLiteNative.sqlc_db_close(mConnectionPtr);
                if (err != SQLiteNative.SQLC_RESULT_OK) throw new SQLiteException("Could not close db");
                mConnectionPtr = 0;
            } finally {
...
*/
        ...
        try {
            ...
            try {
                if (outStatementInfo != null) {
                    ...
                    final int columnCount = SQLiteNative.sqlc_st_column_count(statement.mStatementPtr);
                    if (columnCount == 0) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    //        mConnectionPtr, statement.mStatementPtr);
                    int sr = SQLiteNative.sqlc_st_step(statement.mStatementPtr);
                    if (sr == SQLiteNative.SQLC_RESULT_ROW) throw new SQLiteException("Row result not expected");
                    if (sr != SQLiteNative.SQLC_RESULT_DONE) throw new SQLiteException("sql statement error");
                    changedRows = SQLiteNative.sqlc_db_changes(statement.mStatementPtr);
                    return changedRows;
                } finally {
                    detachCancellationSignal(cancellationSignal);
                }
    ...
    private PreparedStatement acquirePreparedStatement(String sql) {
        ...
        final long statementPtr = SQLiteNative.sqlc_db_prepare_st(mConnectionPtr, sql);
        if (statementPtr < 0) throw new RuntimeException("could not prepare statement: " + (-statementPtr));
        try {
            //final int numParameters = nativeGetParameterCount(mConnectionPtr, statementPtr);
            final int numParameters = SQLiteNative.sqlc_st_bind_parameter_count(statementPtr);
    ...
    private void releasePreparedStatement(PreparedStatement statement) {
        ...
        if (statement.mInCache) {
            try {
                ...
                int err = SQLiteNative.sqlc_st_reset(statement.mStatementPtr);
                if (err != SQLiteNative.SQLC_RESULT_OK) throw new SQLiteException("reset statement error");
                err = SQLiteNative.sqlc_st_clear_bindings(statement.mStatementPtr);
                if (err != SQLiteNative.SQLC_RESULT_OK) throw new SQLiteException("reset statement error");
            } catch (SQLiteException ex) {
    ...
    private void finalizePreparedStatement(PreparedStatement statement) {
        ...
        SQLiteNative.sqlc_st_finish(statement.mStatementPtr);
...
/* XXX TODO:
        if (!mConfiguration.isInMemoryDb()) {
            if (statement.mReadOnly) {
                android.os.StrictMode.getThreadPolicy().onReadFromDisk();
            } else {
                android.os.StrictMode.getThreadPolicy().onWriteToDisk();
            }
        }
*/
    }
